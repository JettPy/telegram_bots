...
cities_button = telebot.types.InlineKeyboardButton("Использованные города:", callback_data='cities_button')
...
inline_menu.add(lose_button, score_button, restart_button, help_button, cities_button)
...
def text(message):
    ...
    if word in cities_used:
        ...
        if next_city is None:
            bot.send_message(message.from_user.id, "У меня закончились идеи. Вы победили.")
            cities_used.clear()
            bot.send_message(message.from_user.id, "Чтобы начать игру, напишите /start", reply_markup=inline_menu)
        else:
...
def button_handlers(call):
    ...
    if call.data == 'lose_button':
        ...
        bot.send_message(call.message.chat.id, f"Вы сдались. Ваш счет: {score}")
        score = 0
        cities_used.clear()
        bot.send_message(call.message.chat.id, "Начинаем игру. Напишите город", reply_markup=inline_menu)
    elif call.data == 'score_button':
        bot.send_message(call.message.chat.id, f"Ваш счет: {score}", reply_markup=inline_menu)
    elif call.data == 'restart_button':
        cities_used.clear()
        score = 0
        bot.send_message(call.message.chat.id, 'Игра перезапущена, начинайте', reply_markup=inline_menu)
    elif call.data == 'help_button':
        bot.send_message(call.message.chat.id, "Подсказки:", reply_markup=reply_menu)
    elif call.data == 'cities_button':
        if cities_used is None:
            bot.send_message(call.message.chat.id, "Список использованных городов пуст", reply_markup=inline_menu)
        else:
            bot.send_message(call.message.chat.id, str(cities_used), reply_markup=inline_menu)
